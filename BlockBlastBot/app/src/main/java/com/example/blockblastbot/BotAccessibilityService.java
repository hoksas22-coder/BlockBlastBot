...
public class BotAccessibilityService extends AccessibilityService {
    ...
    static class Vision {
        ...
        static boolean sampleBlock(Bitmap b, float x, float y, float sx, float sy, int bg) {
            int cx=Math.round(x*sx), cy=Math.round(y*sy);
            int rad=Math.max(5,Math.round(18*Math.min(sx,sy)));
            int hits=0,total=0;
            for(int yy=-rad;yy<=rad;yy+=Math.max(2,rad/5)) for(int xx=-rad;xx<=rad;xx+=Math.max(2,rad/5)) {
                int px=Math.max(0,Math.min(b.getWidth()-1,cx+xx)), py=Math.max(0,Math.min(b.getHeight()-1,cy+yy));
                int c=b.getPixel(px,py); total++;
                float d=dist(c,bg), dl=luma(c)-luma(bg);
                if(d>32f && dl>8f) hits++;
            }
            return hits*100 >= total*42;
        }
        ...
        static List<Piece> readPieces(Bitmap b) {
            ...
            float sx=b.getWidth()/921f, sy=b.getHeight()/2048f;
            int left=Math.max(0,Math.round(70*sx)), right=Math.min(b.getWidth(),Math.round(850*sx));
            int top=Math.max(0,Math.round(1330*sy)), bottom=Math.min(b.getHeight(),Math.round(1650*sy));
            if(right<=left||bottom<=top)return out;
            Bitmap crop=Bitmap.createBitmap(b,left,top,right-left,bottom-top);
            try {
                // Use a dynamic background reference. This survives blue -> red ->
                // yellow -> purple, etc., because only difference from the tray
                // background matters.
                int c1=meanColor(crop,0,0,crop.getWidth()/5,crop.getHeight()/5);
                int c2=meanColor(crop,crop.getWidth()*4/5,0,crop.getWidth(),crop.getHeight()/5);
                int bg=Color.rgb((Color.red(c1)+Color.red(c2))/2,(Color.green(c1)+Color.green(c2))/2,(Color.blue(c1)+Color.blue(c2))/2);
                int step=Math.max(2,Math.round(2*Math.min(sx,sy)));
                int gw=(crop.getWidth()+step-1)/step, gh=(crop.getHeight()+step-1)/step;
                boolean[][] on=new boolean[gh][gw], seen=new boolean[gh][gw];
                for(int gy=0;gy<gh;gy++) for(int gx=0;gx<gw;gx++){
                    int px=Math.min(crop.getWidth()-1,gx*step+step/2), py=Math.min(crop.getHeight()-1,gy*step+step/2);
                    int c=crop.getPixel(px,py);
                    on[gy][gx]=dist(c,bg)>30f && luma(c)>luma(bg)+6f;
                }
                ArrayList<TrayCell> cells=new ArrayList<>();
                int[] dx={1,-1,0,0},dy={0,0,1,-1}; ArrayDeque<int[]> q=new ArrayDeque<>();
                for(int gy=0;gy<gh;gy++) for(int gx=0;gx<gw;gx++){
                    if(!on[gy][gx]||seen[gy][gx])continue;
                    seen[gy][gx]=true;q.add(new int[]{gx,gy});int count=0,minx=gx,maxx=gx,miny=gy,maxy=gy;
                    while(!q.isEmpty()){
                        int[]v=q.removeFirst();int x0=v[0],y0=v[1];count++;minx=Math.min(minx,x0);maxx=Math.max(maxx,x0);miny=Math.min(miny,y0);maxy=Math.max(maxy,y0);
                        for(int d=0;d<4;d++){int nx=x0+dx[d],ny=y0+dy[d];if(nx>=0&&nx<gw&&ny>=0&&ny<gh&&on[ny][nx]&&!seen[ny][nx]){seen[ny][nx]=true;q.add(new int[]{nx,ny});}}
                    }
                    if(count>=80){
                        float px=left+((minx+maxx+1)/2f)*step, py=top+((miny+maxy+1)/2f)*step;
                        cells.add(new TrayCell(px/sx,py/sy));
                    }
                }
                cells.sort((a,c)->Float.compare(a.x,c.x));
                ArrayList<ArrayList<TrayCell>> groups=new ArrayList<>();
                for(TrayCell c:cells){ArrayList<TrayCell> g=groups.isEmpty()?null:groups.get(groups.size()-1);float mx=-Float.MAX_VALUE;if(g!=null)for(TrayCell z:g)mx=Math.max(mx,z.x);if(g==null||c.x-mx>115f){g=new ArrayList<>();groups.add(g);}g.add(c);}
                if(groups.size()>3){groups.sort((a,c)->Integer.compare(c.size(),a.size()));while(groups.size()>3)groups.remove(groups.size()-1);groups.sort((a,c)->Float.compare(avgX(a),avgX(c)));}
                for(ArrayList<TrayCell> g:groups){
                    if(g.isEmpty()||g.size()>5)continue;
                    ...
                    float grid=46f;int cols=Math.max(1,Math.min(5,Math.round((maxX-minX)/grid)+1)),rows=Math.max(1,Math.min(5,Math.round((maxY-minY)/grid)+1));
                    boolean[][] a=new boolean[rows][cols];
                    for(TrayCell c:g){int cc=Math.max(0,Math.min(cols-1,Math.round((c.x-minX)/grid))),rr=Math.max(0,Math.min(rows-1,Math.round((c.y-minY)/grid)));a[rr][cc]=true;}
                    Piece p=new Piece();p.h=rows;p.w=cols;p.a=a;p.minR=0;p.minC=0;
                    // Pick a real occupied tile near the group centroid as the DOWN point.
                    float ax=avgX(g),ay=avgY(g),best=Float.MAX_VALUE;
                    for(TrayCell c:g){float d=(c.x-ax)*(c.x-ax)+(c.y-ay)*(c.y-ay);if(d<best){best=d;p.cx=c.x;p.cy=c.y;p.anchorC=Math.max(0,Math.min(cols-1,Math.round((c.x-minX)/grid)));p.anchorR=Math.max(0,Math.min(rows-1,Math.round((c.y-minY)/grid)));}}
                    out.add(p);
                }
            }finally{crop.recycle();}
            out.sort((a,c)->Float.compare(a.cx,c.cx));return out;
        }
