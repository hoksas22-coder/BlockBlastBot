...
public class BotAccessibilityService extends AccessibilityService {
    ...
    public static void testTouch(TestCallback cb) {
        ...
        s.main.post(() -> {
            ...
            p.lineTo(Math.min(dm.widthPixels - 2f, x + 1f), y);
    ...
    @Override public void onServiceConnected() {
        ...
        try {
            ...
            Log.d(TAG, "ACCESSIBILITY CONNECTED; canPerformGestures=true");
        } catch (Throwable t) {
            Log.e(TAG, "ACCESSIBILITY INFO ERROR", t);
        }
    }
    ...
    private boolean drag(float x1, float y1, float x2, float y2) {
        ...
        main.post(() -> {
            ...
            try {
                // IMPORTANT: use one continuous Stroke. The previous version used
                // a zero-length "hold" stroke + continuation. Some Android builds
                // accept dispatchGesture() but never generate a real DOWN event
                // for that construction. A tiny 1px movement forces a real touch.
                Path path = new Path();
                path.moveTo(fx1, fy1);

                // Hold at the piece for ~450 ms while moving only 1 px.
                float hx = Math.min(maxX, fx1 + 1f);
                path.lineTo(hx, fy1);

                // Smooth movement through an intermediate point.
                float mx = fx1 + (fx2 - fx1) * 0.35f;
                float my = fy1 + (fy2 - fy1) * 0.35f;
                path.lineTo(mx, my);
                path.lineTo(fx2, fy2);

                GestureDescription.StrokeDescription stroke =
                        new GestureDescription.StrokeDescription(path, 0, 1100, false);
                ...
                boolean dispatched = dispatchGesture(gesture, new GestureResultCallback() {
                    @Override public void onCompleted(GestureDescription gd) {
                        ok.set(true);
                        Log.d(TAG, "DRAG COMPLETED " + fx1 + "," + fy1 + " -> " + fx2 + "," + fy2);
                        done.countDown();
                    }
                    @Override public void onCancelled(GestureDescription gd) {
                        Log.e(TAG, "DRAG CANCELLED " + fx1 + "," + fy1 + " -> " + fx2 + "," + fy2);
                        done.countDown();
                    }
                }, main);

                if (!dispatched) {
                    Log.e(TAG, "DRAG REJECTED: dispatchGesture=false");
                    done.countDown();
                }
            } catch (Throwable t) {
        ...
        try {
            if (!done.await(3500, java.util.concurrent.TimeUnit.MILLISECONDS)) {
