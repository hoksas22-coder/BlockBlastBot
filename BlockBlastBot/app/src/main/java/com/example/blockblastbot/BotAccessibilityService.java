...
public class BotAccessibilityService extends AccessibilityService {
    ...
    private void process(Bitmap bm) {
        ...
        // Do not issue another drag while the previous move is still being
        // animated by Block Blast.  The screen capture can contain several
        // identical frames while the piece is moving/disappearing.
        long now = System.currentTimeMillis();
        if (now - lastMoveAt < 1400) { bm.recycle(); return; }
        ...
        // A player can place a piece manually. After that the tray legitimately
        // contains only 2 (or 1) pieces. Requiring exactly 3 made the bot stop
        // permanently after a single manual move. Work with any 1..3 pieces.
        if (pieces.isEmpty()) { bm.recycle(); return; }

        Solver.Move m = Solver.bestSingle(board, pieces);
        if (m == null) { bm.recycle(); return; }

        int idx = m.pieceIndex;
        int row = m.row, col = m.col;
        Piece piece = pieces.get(idx);
        float sx = bm.getWidth() / 921f, sy = bm.getHeight() / 2048f;

        // Grab a real occupied tile. Never use the mathematical centroid if it
        // lies in the empty hole of an L/T/Z-shaped piece.
        int grabR = piece.anchorR;
        int grabC = piece.anchorC;
        float x = (piece.originX + (grabC + .5f) * 46f) * sx;
        float y = (piece.originY + (grabR + .5f) * 46f) * sy;

        // The held Block Blast piece is rendered above the finger. Compensate
        // for that lift when releasing, while keeping the grabbed tile aligned
        // with the selected board cell.
        ...
        Log.d(TAG, "move: piece=" + idx + " placed=" + moved +
    ...
    static class Solver {
        static class Move {
            int pieceIndex, row, col, score;
        }

        // Evaluate ONE move from the CURRENT frame. This is intentional:
        // after every successful placement the board/tray changes, so the next
        // move must be calculated from a fresh screenshot. It also works when
        // the user has manually placed one or two of the pieces.
        static Move bestSingle(Board b, List<Piece> ps) {
            Move best = null;
            for (int pi=0; pi<ps.size(); pi++) {
                Piece p = ps.get(pi);
                for (int r=0; r<=8-p.h; r++) {
                    for (int c=0; c<=8-p.w; c++) {
                        if (!fits(b,p,r,c)) continue;
                        Board t=copy(b);
                        int cleared=placeAndCountClears(t,p,r,c);
                        int adjacency=adjacency(t);
                        int holes=localHoles(t);
                        // Lines are by far the most valuable event. When there is
                        // no clear, prefer connected placements over arbitrary
                        // top-left ties; this avoids the old "always corner"
                        // behaviour.
                        int centerPenalty=Math.abs((r+p.h/2)-3)*2 + Math.abs((c+p.w/2)-3)*2;
                        int score=cleared*10000 + adjacency*40 - holes*8 - centerPenalty;
                        if (best==null || score>best.score) {
                            best=new Move(); best.pieceIndex=pi; best.row=r; best.col=c; best.score=score;
                        }
                    }
                }
            }
        ...
        static boolean fits(Board b,Piece p,int r,int c){
            for(int y=0;y<p.h;y++) for(int x=0;x<p.w;x++)
                if(p.a[y][x] && b.a[r+y][c+x]) return false;
            return true;
        }

        static int placeAndCountClears(Board b,Piece p,int r,int c){
            for(int y=0;y<p.h;y++) for(int x=0;x<p.w;x++)
                if(p.a[y][x]) b.a[r+y][c+x]=true;
            boolean[] rr=new boolean[8], cc=new boolean[8];
            int n=0;
            for(int y=0;y<8;y++){boolean full=true;for(int x=0;x<8;x++)if(!b.a[y][x]){full=false;break;}if(full){rr[y]=true;n++;}}
            for(int x=0;x<8;x++){boolean full=true;for(int y=0;y<8;y++)if(!b.a[y][x]){full=false;break;}if(full){cc[x]=true;n++;}}
            for(int y=0;y<8;y++)if(rr[y])for(int x=0;x<8;x++)b.a[y][x]=false;
            for(int x=0;x<8;x++)if(cc[x])for(int y=0;y<8;y++)b.a[y][x]=false;
            return n;
        }

        static int adjacency(Board b){
            int n=0;
            for(int r=0;r<8;r++)for(int c=0;c<8;c++)if(b.a[r][c]){
                if(r<7&&b.a[r+1][c])n++;
                if(c<7&&b.a[r][c+1])n++;
            }
            return n;
        ...
        static int localHoles(Board b){
            int n=0;
            for(int r=0;r<8;r++)for(int c=0;c<8;c++)if(!b.a[r][c]){
                int filled=0,total=0;
                for(int dr=-1;dr<=1;dr++)for(int dc=-1;dc<=1;dc++)if(dr!=0||dc!=0){
                    int rr=r+dr,cc=c+dc;
                    if(rr>=0&&rr<8&&cc>=0&&cc<8){total++;if(b.a[rr][cc])filled++;}
                }
                if(filled>=5&&total>=5)n++;
            }
            return n;
        }
