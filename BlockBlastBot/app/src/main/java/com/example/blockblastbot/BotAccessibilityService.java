...
public class BotAccessibilityService extends AccessibilityService {
    ...
    private boolean drag(float x1, float y1, float x2, float y2) {
        ...
        // Accessibility gesture coordinates are physical screen pixels, exactly
        // like the MediaProjection frame. Keep the coordinates in that same
        // coordinate system and perform a real long-press before moving.
        ...
        main.post(() -> {
            ...
            try {
                // Use two continued strokes. A zero-length segment inside one
                // long stroke is not a reliable long-press on all Android versions.
                Path holdPath = new Path();
                holdPath.moveTo(fx1, fy1);
                GestureDescription.StrokeDescription hold =
                        new GestureDescription.StrokeDescription(holdPath, 0, 380, true);

                Path movePath = new Path();
                movePath.moveTo(fx1, fy1);
                movePath.lineTo(fx2, fy2);
                GestureDescription.StrokeDescription move = hold.continueStroke(movePath, 380, 650, false);
                ...
                if (!dispatched) {
                    Log.e(TAG, "GESTURE REJECTED: dispatchGesture=false. Accessibility service cannot perform gestures.");
                    done.countDown();
                }
            } catch (Throwable t) {
        ...
        try {
            if (!done.await(3000, java.util.concurrent.TimeUnit.MILLISECONDS)) {
                Log.e(TAG, "GESTURE TIMEOUT");
            }
