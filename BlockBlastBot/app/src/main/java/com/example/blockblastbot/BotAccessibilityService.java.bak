...
public class BotAccessibilityService extends AccessibilityService {
    ...
    private void process(Bitmap bm) {
        ...
        // Start at the TRUE geometric center of this particular shape.
        // The center moves when the shape changes, so it must never be hard-coded.
        float x = piece.cx * sx, y = piece.cy * sy;

        // Keep the same relative grab point when moving onto the board.  Using the
        // centroid rather than a fixed anchor tile makes the placement independent
        // of whether the piece is a T, zig-zag, line, corner, etc.
        float centroidCellX=0f, centroidCellY=0f;
        int nCells=0;
        for(int py=0;py<piece.h;py++) for(int px=0;px<piece.w;px++) if(piece.a[py][px]){
            centroidCellX += px + .5f;
            centroidCellY += py + .5f;
            nCells++;
        }
        centroidCellX/=Math.max(1,nCells);
        centroidCellY/=Math.max(1,nCells);

        // Block Blast lifts the held piece above the finger. The finger therefore
        // finishes about one cell below the desired shape centroid.
        final float LIFT_OFFSET = 93f;
        float tx = (board.x + (col + centroidCellX) * board.cellW) * sx;
        float ty = (board.y + (row + centroidCellY) * board.cellH + LIFT_OFFSET) * sy;
    ...
    static class Vision {
        ...
        static List<Piece> readPieces(Bitmap b) {
            ...
            final float[] slotCenters = {176f, 438f, 661f};
            final float[] ys = {1322f, 1368f, 1414f, 1460f, 1506f};
            final float pitch = 46f;

            for (int slot=0; slot<3; slot++) {
                ArrayList<TrayCell> cells=new ArrayList<>();
                float cx=slotCenters[slot];
                for (int ix=-2; ix<=2; ix++) {
                    float xx=cx + ix*pitch;
                    for (float yy:ys) {
                        if (isTrayCell(b,xx,yy,sx,sy)) cells.add(new TrayCell(xx,yy));
                    }
                }

                // A real piece has 1..5 cells.
                ...
                int anchorC=Math.max(0,Math.min(cols-1,Math.round((centerX-minX)/pitch)));
                int anchorR=Math.max(0,Math.min(rows-1,Math.round((centerY-minY)/pitch)));

                Piece p=new Piece();
                p.h=rows; p.w=cols; p.a=a; p.minR=0; p.minC=0;
                p.cx=centerX; p.cy=centerY;
